...
export async function createDataFile(
  ...
  const constName = collection.type === "singleton" ? generateSingletonConstName(collection.typeName) : generateArrayConstName(collection.name);
  const dataPath = path.join(
    directory,
    `${constName}.${extension}`,
  );

  const value = collection.type === "singleton" ? collection.documents[0]?.document : collection.documents.map((doc) => doc.document);
...
type JavaScriptFileConfiguration = {
  collections: Array<Pick<TransformedCollection, "name" | "type" | "typeName">>;
};
...
async function createJavaScriptFile(
  ...
  const exports = configuration.collections.map((collection) =>
    collection.type === "singleton" ? generateSingletonConstName(collection.typeName) : generateArrayConstName(collection.name)
  );
