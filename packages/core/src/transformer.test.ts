...
describe("transform", () => {
  ...
  it("should fire document-skipped event", async () => {
    const posts = defineCollection({
      ...
      transform: (document, { skip }) => {
        if (document.name !== "Two") {
          skip(`Skipping document ${document.name}`);
        }
        return document;
      },
    });
    ...
    emitter.on("transformer:document-skipped", (event) => reasons.push(`${event.filePath}: ${event.reason}`));
    ...
    expect(reasons[0]).toBe("tests/001.md: Skipping document One");
    expect(reasons[1]).toBe("tests/nested/003.md: Skipping document Three");
  });
  ...
  it("should fire document-skipped event without reason", async () => {
    ...
    const reasons: Array<string> = [];
    emitter.on("transformer:document-skipped", (event) => reasons.push(event.reason || "No reason provided"));
