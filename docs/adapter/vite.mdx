...
- [Remix / React Router v7](/docs/adapter/remix)
...
The name of the Vite environment in which the plugin should run. All other environments will be ignored. If you're using a framework that runs the build for multiple environments (for example, `ssr` and `client`), you can use this option to run the plugin only for the `ssr` environment. This way, you avoid running the plugin twice and improve build performance.
