...
1. Now we can add the Content Collections plugin to the Next.js config:
   ...
   <Notification type="warning" prose>
     Ensure that `withContentCollections` is the last plugin in the chain. This is important because the plugin returns a `Promise`, which is compatible with [Next.js](https://nextjs.org/docs/messages/promise-in-next-config#possible-ways-to-fix-it), but other plugins may not expect this behavior.
