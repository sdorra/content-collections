...
Since version 0.10.0, the `transform` function can skip a document from being added to the collection. This feature is useful for filtering out documents that do not meet specific criteria or for preventing certain documents from being processed. To skip a document, simply call the `skip` function of the context object with an optional reason, for example:

```ts
transform: (doc, { skip }) => {
  if (doc.draft) {
    skip("document is a draft");
  }
  return doc;
});
```
